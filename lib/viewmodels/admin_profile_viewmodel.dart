...
class AdminProfileViewModel extends ChangeNotifier {
  ...
  int _uploadedLetters = 0;
  int get uploadedLetters => _uploadedLetters;
  ...
  Future<void> _loadStats() async {
    ...
    try {
      ...
      final currentSessionEnd = DateTime(currentSessionStartYear + 1, 8, 1);
      ...
      for (final event in events) {
        if (event.hasPaperwork) letterCount++;

        // Check if event is in current session
        if (event.date.isAfter(currentSessionStart) && event.date.isBefore(currentSessionEnd)) {
          sessionCount++;
          ...
          if (event.date.isAfter(sem2Start) || event.date.isAtSameMomentAs(sem2Start)) {
            sem2Count++;
          }
        }
      }
